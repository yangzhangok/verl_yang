        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                        ...
                        if "multi_modal_data" in shuffled_gen_batch.non_tensor_batch:
                            ...
                            for batch_idx in range(batch_size):
                                ...
                                if len(image_token_positions) > 0:
                                    ...
                                    # Now we expect image_embeds to be already processed by the dataset
                                    if "image_embeds" in image_data:
                                        image_embeds = image_data["image_embeds"]
                                        num_image_tokens = len(image_token_positions)
                                        
                                        # After dataset processing, image_embeds should have the same number of rows as image tokens
                                        ...
                                        print(f"Warning: Shuffle {shuffle_seed}, Sample {batch_idx}: No image_embeds found in multi_modal_data")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # MODIFIED: Skip reward and advantage computation, directly use cross-entropy loss
                    print("Using direct cross-entropy loss training (skipping reward/advantage computation)")
                    
                    # Only compute log_probs for cross-entropy loss calculation
                    with marked_timer("log_prob", timing_raw, color="blue"):
                        log_prob_result = self.actor_rollout_wg.compute_log_prob(batch)
                        entropys = log_prob_result.batch["entropys"]
                        response_masks = batch.batch["response_mask"]
                        loss_agg_mode = self.config.actor_rollout_ref.actor.loss_agg_mode
                        entropy_agg = agg_loss(loss_mat=entropys, loss_mask=response_masks, loss_agg_mode=loss_agg_mode)
                        log_prob_metrics = {"actor/entropy": entropy_agg.detach().item()}
                        metrics.update(log_prob_metrics)
                        log_prob_result.batch.pop("entropys")
                        batch = batch.union(log_prob_result)

                    # Create dummy advantages for compatibility (all zeros)
                    # This allows the existing update_actor function to work without modification
                    batch_size = batch.batch["responses"].shape[0]
                    response_length = batch.batch["responses"].shape[1]
                    dummy_advantages = torch.zeros_like(batch.batch["old_log_probs"])
                    batch.batch["advantages"] = dummy_advantages
                    
                    # Create dummy old_log_probs if not present
                    if "old_log_probs" not in batch.batch:
                        batch.batch["old_log_probs"] = batch.batch["log_probs"].detach()

                    # Skip critic update for cross-entropy training
                    print("Skipping critic update for cross-entropy training")

                    # Direct actor update with cross-entropy loss
                    with marked_timer("update_actor", timing_raw, color="red"):
                        batch.meta_info["multi_turn"] = self.config.actor_rollout_ref.rollout.multi_turn.enable
                        # Set a flag to indicate cross-entropy training mode
                        batch.meta_info["cross_entropy_training"] = True
                    ...
                    if rollout_data_dir:
                        # Create empty reward_extra_infos_dict for logging compatibility
                ...
                # collect metrics
                metrics.update(compute_data_metrics_safe(batch=batch, use_critic=self.use_critic))
